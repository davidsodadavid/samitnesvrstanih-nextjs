...
/**
 * Film events whose description contains a YouTube link, newest first.
 *
 ...
 */
export async function getFilmVideos(limit = 6): Promise<FilmVideo[]> {
  const events = await prisma.event.findMany({
    where: { event_type: { name: { equals: FILM_TYPE_NAME, mode: 'insensitive' } } },
