...
export function Navbar({ galleryYears }: { galleryYears: number[] }) {
  // ARCHIVE points at the most recent gallery year until there's an index page.
  const archiveHref = galleryYears.length > 0 ? `/galleries/${galleryYears[0]}` : null
        ...
        {navItems.map((item, i) => {
          ...
          const content = (
            <span className="flex items-center gap-1 whitespace-nowrap lg:gap-1.5 xl:gap-2">
              <img src={item.desktopIcon} alt="" className="h-4 w-auto lg:h-5 xl:h-6 2xl:h-7" />
